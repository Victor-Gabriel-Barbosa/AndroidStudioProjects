...
class UsuarioFirebase(
     ...
     *
     * @param usuario O objeto [com.example.mapa.data.remote.dto.UsuarioDTO] a ser salvo.
     ...
     *
     * @return Um [kotlinx.coroutines.flow.Flow] que emite uma lista de [UsuarioDTO] sempre que houver atualizações.
     */
     ...
     * @return Um [Flow] que emite um objeto [UsuarioDTO] sempre que houver uma atualização.
     */
    override fun findByUid(uid: String): Flow<UsuarioDTO?> = callbackFlow {
        val listener = collection.document(uid).addSnapshotListener { snapshot, error ->
            ...
            if (snapshot != null && snapshot.exists()) trySend(snapshot.toObject(UsuarioDTO::class.java))
            else trySend(null)
        }
